...
from app.services import context_management_runtime, embeddings_service, platform_service
...
def test_query_knowledge_base_rejects_active_provider_instance_mismatch(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(embeddings_service, "embed_text_inputs", lambda *_args, **_kwargs: {"embeddings": [[0.1, 0.2]]})
    monkeypatch.setattr(
        platform_service,
