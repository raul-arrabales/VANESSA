...
from .runtime import generate_for_role


def _service_role() -> str:
    role = os.getenv("IMAGE_GENERATION_ROLE", ROLE_GATEWAY).strip().lower() or ROLE_GATEWAY
...
class Handler(BaseHTTPRequestHandler):
    server_version = "VANESSAImageGeneration/0.1"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if payload is None:
            ...
            return
        result, status = generate_for_role(payload, _service_role())
        self._send_json(result, status)

    def log_message(self, format: str, *args: Any) -> None:  # noqa: A002
        if os.getenv("IMAGE_GENERATION_ACCESS_LOG", "").strip().lower() in {"1", "true", "yes", "on"}:
            super().log_message(format, *args)


def main() -> None:
    port = int(os.getenv("IMAGE_GENERATION_PORT", str(DEFAULT_PORT)))
    server = ThreadingHTTPServer(("0.0.0.0", port), Handler)
    print(f"image_generation role={_service_role()} listening on :{port}", flush=True)
