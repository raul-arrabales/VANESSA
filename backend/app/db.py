...
def run_modelops_schema_migration(database_url: str) -> None:
    with get_connection(database_url) as connection:
        with connection.cursor() as cursor:
            for migration_name in ("008_modelops_domain.sql", "009_modelops_cleanup.sql"):
                migration_file = (
                    Path(__file__).resolve().parents[2]
                    / "infra"
                    / "postgres"
...
def run_modelops_testing_schema_migration(database_url: str) -> None:
    migration_file = (
        Path(__file__).resolve().parents[2]
        / "infra"
        / "postgres"
        / "init"
        / "010_modelops_testing.sql"
    )
    migration_sql = migration_file.read_text(encoding="utf-8")
...
def run_chat_conversations_schema_migration(database_url: str) -> None:
    ...
    migration_files = [
        migration_root / "007_chat_conversations.sql",
        migration_root / "020_chat_attachments.sql",
    ]
