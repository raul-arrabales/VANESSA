...
def _serialize_catalog_tool_execution(
    *,
    ...
    runtime_log = _build_catalog_tool_runtime_log(
        tool=tool,
...
def _catalog_runtime_log_entry(stage: str, message: str, *, level: str = "info", details: dict[str, Any] | None = None) -> dict[str, Any]:
    entry: dict[str, Any] = {
        "stage": stage,
        "level": level,
        "message": message,
...
def _tool_runtime_input_summary(tool: dict[str, Any], input_payload: dict[str, Any]) -> dict[str, Any]:
    spec = tool.get("spec") if isinstance(tool.get("spec"), dict) else {}
    backend = tool_execution_backend(spec) or _TOOL_BACKEND_INTERNAL_HTTP
    summary: dict[str, Any] = {"backend": backend}
    if backend == _TOOL_BACKEND_IMAGE_ANALYSIS:
    ...
    elif backend == _TOOL_BACKEND_IMAGE_GENERATION:
        tasks = input_payload.get("tasks")
        if isinstance(tasks, list):
            summary["tasks"] = [str(task).strip().lower() for task in tasks if str(task).strip()]
    ...
    elif backend == _TOOL_BACKEND_SANDBOX:
    ...
    elif backend == _TOOL_BACKEND_WEB_SEARCH:
    ...
    elif backend == _TOOL_BACKEND_KB_RETRIEVAL:
...
def _result_warning_summary(result_payload: dict[str, Any] | None) -> dict[str, Any] | None:
...
def _build_catalog_tool_runtime_log(
    *,
    tool: dict[str, Any],
    input_payload: dict[str, Any],
    request_metadata: dict[str, Any],
    ...
    spec = tool.get("spec") if isinstance(tool.get("spec"), dict) else {}
    backend = tool_execution_backend(spec) or _TOOL_BACKEND_INTERNAL_HTTP
    summary = _tool_runtime_input_summary(tool, input_payload)
    actor_user_id = request_metadata.get("actor_user_id")
    logs = [
        _catalog_runtime_log_entry(
            "request_received",
    ...
    warning_summary = _result_warning_summary(result_payload)
    if warning_summary:
        logs.append(
            _catalog_runtime_log_entry(
                "runtime_warnings",
