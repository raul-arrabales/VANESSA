...
def get_active_capability_statuses(database_url: str, config: AuthConfig) -> list[dict[str, Any]]:
    ...
    try:
        image_adapter = resolve_image_analysis_adapter(database_url, config)
        statuses.append(
            {
                "capability": CAPABILITY_IMAGE_ANALYSIS,
                "provider": {
                    "id": image_adapter.binding.provider_instance_id,
                    "slug": image_adapter.binding.provider_slug,
                    "provider_key": image_adapter.binding.provider_key,
                    "provider_origin": image_adapter.binding.provider_origin,
                    "display_name": image_adapter.binding.provider_display_name,
                },
                "deployment_profile": {
                    "id": image_adapter.binding.deployment_profile_id,
                    "slug": image_adapter.binding.deployment_profile_slug,
