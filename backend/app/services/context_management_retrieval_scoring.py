...
"""Backend KB retrieval scoring and fusion helpers; canonical contract: docs/services/retrieval_contract.md."""
...
from .context_management_retrieval_types import (
    ...
    KnowledgeBaseRetrievalRelevanceKind,
)
...
def normalize_branch_result_relevance(
    result: KnowledgeBaseRetrievalBranchResult,
    *,
    search_method: str,
) -> tuple[float, KnowledgeBaseRetrievalRelevanceKind]:
...
def rank_branch_results(
    results: list[KnowledgeBaseRetrievalBranchResult],
    *,
    search_method: str,
) -> list[KnowledgeBaseRankedRetrievalResult]:
...
def normalize_keyword_relevance_scores(
...
def fuse_hybrid_results(
    semantic_results: list[KnowledgeBaseRankedRetrievalResult],
    keyword_results: list[KnowledgeBaseRankedRetrievalResult],
    *,
    hybrid_alpha: float,
    top_k: int,
) -> list[KnowledgeBaseRankedRetrievalResult]:
    ...
    fused_results: list[KnowledgeBaseRankedRetrievalResult] = []
    ...
    for result_id in ordered_ids:
        ...
        base_result = semantic_by_id.get(result_id) or keyword_by_id.get(result_id)
