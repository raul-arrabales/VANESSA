...
from ...application.platform_control_service import (
    ...
    delete_platform_deployment,
    delete_platform_provider,
    ...
    list_platform_provider_families,
...
list_capabilities = list_platform_capabilities
list_providers = list_platform_providers
list_provider_families = list_platform_provider_families
create_provider = create_platform_provider
update_provider = update_platform_provider
delete_provider = delete_platform_provider
validate_provider = validate_platform_provider
clear_provider_loaded_model = clear_platform_provider_loaded_model
list_deployment_profiles = list_platform_deployments
...
def assign_provider_loaded_model(
    ...
    *,
    ...
    return assign_platform_provider_loaded_model(
        database_url,
        config=config,
        provider_instance_id=provider_instance_id,
        payload={"managed_model_id": managed_model_id},
    )
...
def create_platform_provider_route():
    try:
        provider = create_provider(_database_url(), config=_config(), payload=request.get_json(silent=True))
...
def ensure_platform_vector_index_route():
    try:
        result = ensure_vector_index(_database_url(), _config(), request.get_json(silent=True))
    except PlatformControlRequestError as exc:
        return _json_error(exc.status_code, exc.code, exc.message)
    except PlatformControlPlaneError as exc:
        return _json_error(exc.status_code, exc.code, exc.message, details=exc.details or None)
...
def platform_embeddings_route():
    try:
        result = embed_platform_inputs(_database_url(), _config(), request.get_json(silent=True))
    except PlatformControlRequestError as exc:
        return _json_error(exc.status_code, exc.code, exc.message)
    except PlatformControlPlaneError as exc:
        return _json_error(exc.status_code, exc.code, exc.message, details=exc.details or None)
...
def upsert_platform_vector_documents_route():
    try:
        result = upsert_vector_documents(_database_url(), _config(), request.get_json(silent=True))
    except PlatformControlRequestError as exc:
        return _json_error(exc.status_code, exc.code, exc.message)
    except PlatformControlPlaneError as exc:
        return _json_error(exc.status_code, exc.code, exc.message, details=exc.details or None)
...
def query_platform_vector_documents_route():
    try:
        result = query_vector_documents(_database_url(), _config(), request.get_json(silent=True))
    except PlatformControlRequestError as exc:
        return _json_error(exc.status_code, exc.code, exc.message)
    except PlatformControlPlaneError as exc:
        return _json_error(exc.status_code, exc.code, exc.message, details=exc.details or None)
...
def delete_platform_vector_documents_route():
    try:
        result = delete_vector_documents(_database_url(), _config(), request.get_json(silent=True))
    except PlatformControlRequestError as exc:
        return _json_error(exc.status_code, exc.code, exc.message)
    except PlatformControlPlaneError as exc:
        return _json_error(exc.status_code, exc.code, exc.message, details=exc.details or None)
