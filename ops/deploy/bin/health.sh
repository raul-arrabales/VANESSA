...
Runs health/liveness checks for the VANESSA deployment selected by VANESSA_DEPLOYMENT_MODE.
USAGE
}
...
llm_contract_ok() {
  curl --silent --show-error --max-time 3 --fail "$(llm_models_probe_url)" | grep -q '"data"'
}
...
llm_runtime_image_mismatch_reason() {
  ...
  if [[ "${actual_image}" != "${expected_image}" ]]; then
    printf 'expected_image=%s, actual_image=%s, hint=use ops/deploy/bin/compose.sh or deployment-mode wrappers\n' "${expected_image}" "${actual_image}"
  fi
}

llama_cpp_ready_ok() { llama_cpp_internal_http_ok "/v1/models"; }
qdrant_ready_ok() { qdrant_internal_http_ok "/healthz"; }
image_analysis_ready_ok() { image_analysis_internal_http_ok "/health"; }
image_analysis_worker_ready_ok() { image_analysis_worker_internal_http_ok "$1" "$2" "/health"; }
image_generation_ready_ok() { image_generation_internal_http_ok "/health"; }
image_generation_worker_ready_ok() { image_generation_worker_internal_http_ok "$1" "$2" "/health"; }
searxng_ready_ok() { searxng_internal_http_ok "/"; }
mcp_gateway_ready_ok() { mcp_gateway_internal_http_ok "/health"; }

runtime_profile_endpoint_ok() {
  local status
  status="$(curl --silent --show-error --max-time 2 -o /dev/null -w '%{http_code}' "$(runtime_profile_probe_url)")"
...
embeddings_slot_runtime_alignment_ok() {
  ...
  local runtime_state_fields runtime_load_state runtime_state_model_id runtime_last_error
  ...
  IFS=$'\t' read -r runtime_load_state runtime_state_model_id _ _ runtime_last_error <<< "${runtime_state_fields}"
...
run_checks() {
  ...
  if http_ok "$(backend_probe_url)"; then
    printf 'backend: OK\n'
  else
    printf 'backend: FAIL\n'
    failures=$((failures + 1))
  ...
  if http_ok "$(agent_engine_probe_url)"; then
  ...
  if http_ok "$(sandbox_probe_url)"; then
    printf 'sandbox: OK\n'
  else
    printf 'sandbox: FAIL\n'
    failures=$((failures + 1))
  ...
  if http_ok "$(llm_probe_url)" && llm_contract_ok; then
  ...
  if llama_cpp_enabled_requested; then
    ...
    printf 'llama_cpp: SKIP (not enabled in VANESSA_ENABLED_OPTIONAL_SERVICES)\n'
  fi
  ...
  if qdrant_enabled_requested; then
    ...
    printf 'qdrant: SKIP (not enabled in VANESSA_ENABLED_OPTIONAL_SERVICES)\n'
  fi
  ...
  if image_analysis_enabled_requested; then
    ...
    printf 'image_analysis: SKIP (not enabled in VANESSA_ENABLED_OPTIONAL_SERVICES)\n'
  fi
  ...
  if image_generation_enabled_requested; then
    ...
    printf 'image_generation: SKIP (not enabled in VANESSA_ENABLED_OPTIONAL_SERVICES)\n'
  fi
  ...
  if web_search_enabled_requested; then
    ...
    printf 'searxng: SKIP (not enabled in VANESSA_ENABLED_OPTIONAL_SERVICES)\n'
  fi
  ...
  if http_ok "$(weaviate_probe_url)"; then
    printf 'weaviate: OK\n'
  else
  ...
  if tcp_ok "$(host_probe_host "${POSTGRES_BIND_HOST:-127.0.0.1}")" "${POSTGRES_PUBLISHED_PORT:-5432}"; then
    printf 'postgres: OK\n'
  else
