...
Starts the VANESSA stack for the selected deployment mode and waits for readiness.
USAGE
}
...
if [[ "${seed_sample_users}" == true && "${VANESSA_ALLOW_SAMPLE_USER_SEEDING}" != "true" ]]; then
  die "--seed-sample-users is only supported in deployment modes that explicitly allow sample auth seeding."
fi
...
log_info "Deployment mode: ${VANESSA_DEPLOYMENT_MODE}"
log_info "Enabled optional services: ${VANESSA_ENABLED_OPTIONAL_SERVICES:-<none>}"
...
if [[ ${status} -eq 0 ]]; then
  if [[ "${seed_sample_users}" == true ]]; then
    ...
    COMPOSE_FILE="${COMPOSE_FILE}" COMPOSE_ENV_FILE="${COMPOSE_ENV_FILE}" VANESSA_DEPLOYMENT_MODE="${VANESSA_DEPLOYMENT_MODE}" "${SCRIPT_DIR}/seed-users.sh" || die "Failed to seed sample users"
  fi
